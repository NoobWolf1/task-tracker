import { DONE, HIGH, IN_PROGRESS, LOW, MEDIUM, MESSAGES, PRIORITY_ARRAY, PROGRESS_ARRAY, TODO, VALID_INPUT } from "../config";
...
export class Validate {
    ...
    public addValidation(input: string, command: string): Validation {
        ...
        const rest = input.slice(quoteEnd + 1).trim().split(" ").filter(Boolean); // what is this? i mean how does it work
        if(rest.length > 2) {
